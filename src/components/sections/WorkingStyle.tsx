...
export function WorkingStyle() {
  return (
    <section id="atuacao" className="mx-auto max-w-7xl px-6 py-8">
      ...
      <Reveal delay={80}>
        <p className="mb-10 text-justify text-base text-muted">
      ...
      <div className="grid items-stretch gap-8 lg:grid-cols-[560px_1fr]">
        ...
        <Reveal
          delay={120}
          className="flex flex-col rounded-xl border border-border bg-gradient-to-br from-surface/80 to-surface/35 p-7 shadow-lg shadow-black/20 transition !duration-200 hover:border-accent/50"
        >
          ...
          <div className="flex flex-1 flex-col justify-between divide-y divide-border">
            {techPillars.map((item) => (
              <div key={item.title} className="py-3 first:pt-0 last:pb-0">
                ...
                <p className="mt-1.5 text-justify text-sm text-muted">{item.items}</p>
              </div>
            ))}
          </div>
        </Reveal>
          ...
          {problemsSolved.map((item, i) => (
            <Reveal key={item.title} delay={i * 80} className="flex gap-5">
              ...
              <div>
                ...
                <p className="mt-1 text-justify text-sm text-muted">{item.description}</p>
              </div>
            </Reveal>
          ))}
        </div>
